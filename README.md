...
- **Round-Robin Scheduler (demo)**: Minimal sched_ext scheduler that simply time-slices tasks and streams the synthetic RAPL stats
...
This scheduler is a minimal Round-Robin policy (same time slice for every runnable task) that simply prints the random values produced by the RAPL stats updater. It is meant as a plumbing test before adding energy-aware decisions.

```bash
sudo make run-energy
```

Make sure `rapl_stats_updater` is running so the loader can attach to the pinned `/sys/fs/bpf/rapl_stats` map. The stats are logged from the kernel via `bpf_printk`, so monitor them with:
...
- `scx_energy_aware.bpf.c` - Minimal Round-Robin sched_ext policy (placeholder for full energy-aware logic)
- `scx_energy_aware_loader.c` - Loader that attaches the scheduler and links the pinned RAPL map so the kernel can log stats via `bpf_printk`
...
3. Use `stats.package_power`, `stats.core_temp[i]`, etc. for scheduling decisions
